...
import { DynamicTableView} from './dynamic_dashboard_table';
...
export class OdooDynamicDashboard extends Component {
    ...
    applySavedTheme() {
        ...
        if (savedTheme) {
            $(".container").attr("style", savedTheme + "min-height:-webkit-fill-available;");
        }
    }
    // getScrollingElement() {
    //     // Fallback for getting the scrolling element
    ...
    // }
    onChangeTheme() {
        /* Function for changing the theme of the dashboard */
        const selectedTheme = this.ThemeSelector.el.value;
        $(".container").attr("style", selectedTheme + "min-height:-webkit-fill-available;");
        ...
        await this.orm.call("dashboard.block", "get_dashboard_vals", [[], this.actionId]).then( function (response){
            for (let i = 0; i < response.length; i++) {
                if (response[i].type === 'tile'){
                    ...
                    const columns = response[i].columns || [];
                    const records = response[i].records || []
                    mount(DynamicTableView, $('.items')[0], {
                        props: {
                            widget: {
                                ...response[i],
                                columns: columns,
    ...
    async printPdf() {
        var tiles = $('.tile_cus');
        var tables = $('.table_cus');
        var charts = $('.chart_cus');
        const savedTheme = localStorage.getItem("dashboardTheme");
        let themeBackground = savedTheme?.match(/linear-gradient\(.*\)/)?.[0];
    
        // Create a new container for the PDF
        var newElement = document.createElement('div');
        newElement.className = 'pdf';

    
        // Style container to ensure proper layout
        newElement.style.width = '100%';
        newElement.style.display = 'flex';
        newElement.style.flexDirection = 'row';
        newElement.style.flexWrap = 'wrap';
        newElement.style.alignItems = 'center';
        // newElement.style.background = themeBackground;

    
        // Helper function to set dimensions and avoid page breaks
        function setDimensionsAndPageBreak(element, width, height) {
        ...
        // Append and style tiles
        tiles.each(function(index, elem) {
            let clonedTile = elem.cloneNode(true);
            setDimensionsAndPageBreak(clonedTile, '240px', '150px');
            newElement.appendChild(clonedTile);
        });

        tables.each((index, elem) => {
            let clonedTable = elem.cloneNode(true);
            setDimensionsAndPageBreak(clonedTable, '100%', 'auto');
        
            // Dynamically resize rows to fit content
            ...
            newElement.appendChild(clonedTable);
        });
        ...
        charts.each((index, elem) => {
            ...
            if (canvas) {
                ...
                // Create an <img> element from the canvas data
                ...
                // Wrap in a div container to maintain layout
        ...
        // Reset transformations to ensure proper rendering
        var children = $(newElement).children();
        children.each(function(index, child) {
            $(child).css('transform', ''); // Reset transformation styles
        });
